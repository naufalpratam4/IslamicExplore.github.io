<div class="md:mx-60">
    <div class="text-5xl font-semibold flex" style="color: #369F43">
        <div class="md:mx-auto mx-5">What do users say?</div>
    </div>
    <div class="grid md:grid-cols-3 my-5 gap-3 md:gap-0 mx-4 md:mx-0">
        <div class="border-4 mx-auto border-green-500 md:w-72 text-center px-5 pt-5 rounded-lg">
            <div>saya sangat terkesan dengan kualitas dan kemudahan yang ditawarkan oleh aplikasi ini. Materi
                pembelajaran
                yang disediakan sangat komprehensif dan disajikan dengan cara yang menarik serta mudah dipahami.</div>
            <div class="flex"><img class="mx-auto mt-5" src="{{ asset('assets/landingpage/usersay.png') }}"
                    alt="">
            </div>
            <div class="font-semibold text-xl mb-1">Haechan</div>
            <div>Secretary</div>
        </div>
        <div class=" mx-auto border-4 border-green-500 md:w-72 text-center px-5 pt-5 rounded-lg">
            <div>saya sangat terkesan dengan kualitas dan kemudahan yang ditawarkan oleh aplikasi ini. Materi
                pembelajaran
                yang disediakan sangat komprehensif dan disajikan dengan cara yang menarik serta mudah dipahami.</div>
            <div class="flex"><img class="mx-auto mt-5" src="{{ asset('assets/landingpage/usersay.png') }}"
                    alt="">
            </div>
            <div class="font-semibold text-xl mb-1">Haechan</div>
            <div>Secretary</div>
        </div>
        <div class="mx-auto border-4 border-green-500 md:w-72 text-center px-5 pt-5 rounded-lg">
            <div>saya sangat terkesan dengan kualitas dan kemudahan yang ditawarkan oleh aplikasi ini. Materi
                pembelajaran
                yang disediakan sangat komprehensif dan disajikan dengan cara yang menarik serta mudah dipahami.</div>
            <div class="flex"><img class="mx-auto mt-5" src="{{ asset('assets/landingpage/usersay.png') }}"
                    alt="">
            </div>
            <div class="font-semibold text-xl mb-1">Haechan</div>
            <div class="pb-5">Secretary</div>
        </div>



    </div>
</div>
